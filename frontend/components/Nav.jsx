...
export default function Nav() {
  ...
  const Menu = ({ label, keyName, items }) => (
    <div className="relative" onMouseLeave={()=> setOpen(o=> ({...o, [keyName]: false}))}>
      <button
      ...
      {open[keyName] && (
        <div className="absolute left-0 mt-2 bg-white dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 rounded-md shadow-lg min-w-48 z-50 p-1"
             onMouseEnter={()=> setOpen(o=> ({...o, [keyName]: true}))}
        >
          {items.map(i=> (
  ...
  return (
    <header className="border-b border-zinc-200 dark:border-zinc-700 bg-white/70 dark:bg-zinc-900/60 backdrop-blur">
