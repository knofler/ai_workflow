...
export async function GET(_req, { params }) {
  const r = await fetch(`${base}/workflows/${params.id}`, { cache: 'no-store' });
...
export async function PUT(req, { params }) {
  ...
  const r = await fetch(`${base}/workflows/${params.id}`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body });
  const data = await r.json();
  return new Response(JSON.stringify(data), { status: r.status, headers: { 'Content-Type': 'application/json' } });
}

export async function DELETE(_req, { params }) {
  const r = await fetch(`${base}/workflows/${params.id}`, { method: 'DELETE' });
