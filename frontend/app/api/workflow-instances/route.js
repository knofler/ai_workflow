...
export async function GET(req) {
  ...
  const r = await fetch(`${base}/workflow-instances${u.search}`, { cache: 'no-store' });
...
export async function POST(req) {
  ...
  const r = await fetch(`${base}/workflow-instances`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body });
  const data = await r.json();
  return new Response(JSON.stringify(data), { status: r.status, headers: { 'Content-Type': 'application/json' } });
}
